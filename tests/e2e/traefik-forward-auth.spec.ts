// Generated from radarr-forward-auth.spec.ts: same flow, Traefik instead of nginx.
 ...
 * correct approach. Here the app is switched to Traefik first, so the same
 * sso:protect goes through the Traefik adapter rather than the nginx one.
 */

const APP = 'test-traefik-auth';
const DOMAIN = `${APP}.test.local`;
const AUTH_SERVICE = 'traefik-auth-dir';
const FRONTEND_SERVICE = 'traefik-auth-fe';
...
test.describe('Traefik Forward Auth Integration', () => {
  ...
  test.beforeAll(async () => {
    ...
    // The point of this spec: the same protection, applied through Traefik.
    // sso:protect re-runs the proxy adapter, which attaches the forwardAuth
    // middleware and rebuilds so the new container picks the labels up.
    dokku(`proxy:set ${APP} type traefik`);
    dokku(`traefik:start`, { swallowErrors: true });
    dokku(`sso:protect ${APP} --service ${AUTH_SERVICE}`, { timeout: 300_000 });
